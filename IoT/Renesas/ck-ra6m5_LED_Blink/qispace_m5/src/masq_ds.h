...
/**
 * Constructs an MASQ_DS handle for a particular Quantropi Digital Signature algorithm.
 *
 ...
 *
 * @param[in] algorithm_name is the Name of the desired algorithm; one of the names in `MASQ_DS_algs`.
 * @param[in] rand_cf is the Random generator call back function. When it is NULL, will use the internal random function.
 * @param[in] rand_seed_cf is the Random seeds call back function. When it is NULL, will use the internal random function.
 * @param[in] rand_handle is the handle for random generator.
 * @return An MASQ_DS_handle for the particular algorithm, or `NULL` if the algorithm has been disabled at compile-time.
 */
MASQ_DS_handle *MASQ_DS_init( MASQ_DS_LEVEL algorithm_name, MASQ_rand_callback_t rand_cf, MASQ_rand_seed_callback_t rand_seed_cf, MASQ_RAND_handle rand_handle );
 ...
 * @return MASQ_SUCCESS or MASQ_ERR_xxx
 */
MASQ_API MASQ_ds_seed( MASQ_DS_handle *ds, uint8_t *seed, int32_t seed_length );
 ...
 * @return MASQ_SUCCESS or MASQ_ERR_xxx
 */
MASQ_API MASQ_DS_keypair(MASQ_DS_handle *ds, uint8_t *public_key, uint8_t *private_key);
 ...
 * @return MASQ_SUCCESS or MASQ_ERR_xxx
 */
MASQ_API MASQ_DS_sign(MASQ_DS_handle *ds, uint8_t *private_key, uint8_t *message, int32_t message_len, uint8_t *signature, int32_t *signature_len );
 ...
 * @return MASQ_SUCCESS or MASQ_ERR_xxx
 */
MASQ_API MASQ_DS_verify(MASQ_DS_handle *ds, uint8_t *public_key, uint8_t *message, int32_t message_len, uint8_t *signature, int32_t signature_len);
...
MASQ_API MASQ_DS_free(MASQ_DS_handle *ds);
