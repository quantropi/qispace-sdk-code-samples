 ...
 * Enterprise to use as seed or use pre-loaded QE in the same format.
*/
...
void print_usage(char *name) {
    std::cout << "Usage: " << name <<" [-h] [--qe QE] [--length len] \n";
    std::cout << "\nOptions:\n";
    std::cout << "  -h, --help        Show this help message and exit\n";
    std::cout << "  --qe              QE hex string to seed PQRND\n";
    std::cout << "  --length          Required length of pseudo quantum random string to generate in bytes\n";
}
...
*/
    ...
    if (ret != QEEP_OK) {
        std::cout <<"Initializing PQRND handle failed, ret = %d \n" << ret;
        return 1;
    }
    std::cout <<"Success!\n";
...
*/
    std::cout <<"Generate PQRND...\n";
    ret = pQrndRnd(qrnd_handle, rnd_out, qe_len);
    if (ret != QEEP_OK) {
        std::cout <<"pQrndRnd fail, ret = %d \n" << ret;
        return 1;
    }
    std::cout <<"Success!\n";

    std::cout <<"Generated Quantum random data: ";
    for (i = 0; i < qe_len ; i++ ) {
